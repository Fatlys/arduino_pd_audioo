...
void setup() {
  // LED config
  pinMode(TOGGLE_LIGHT_PIN, OUTPUT);
  digitalWrite(TOGGLE_LIGHT_PIN, LOW);
  pinMode(MOMENTARY_LIGHT_PIN, OUTPUT);
  digitalWrite(MOMENTARY_LIGHT_PIN, LOW);
...
void loop() {
  ...
  if (toggleButton.pressed() || toggleButton.released()) {
    bool pressed = toggleButton.isPressed();
 
    digitalWrite(TOGGLE_LIGHT_PIN, pressed);
